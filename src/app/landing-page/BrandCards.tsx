...
const BrandCards = () => {
  return (
    <div
      className={`w-full  py-6 flex flex-col gap-4 ${passion_one.className}`}
    >
      <h2 className="font-bold text-center text-4xl text-[#EA1D2F] uppercase tracking-[-5%]">
        Our past work
      </h2>
      <div className="grid lg:grid-cols-4 md:grid-cols-2 sm:flex flex-col items-center  gap-3 px-6 sm:px-2">
        {cards.map((card, index) => (
          ...
          >
            ...
            {/* Title */}
            <h2
              className={` text-[#D01717] mt-3 text-center text-4xl leading-none font-normal uppercase`}
            >
              {card.title}
            </h2>
